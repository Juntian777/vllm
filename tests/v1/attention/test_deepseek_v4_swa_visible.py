...
def test_dflash_refresh_replays_after_eager_build_and_batch_shrink(query_len, tmp_path):
    """Graph-owned mappings survive eager builds; group validity stays independent."""
    ...
    with torch.cuda.graph(graph):
        ...
        outputs = [
            (
                m.decode_swa_indices.clone(),
                m.decode_swa_lens.clone(),
                m.is_valid_token.clone(),
            )
            for m in metadata
        ]
    for step, active in enumerate((4, 1, 3, 4)):
        ...
        for group, (builder, cm, md, actual) in enumerate(
            ...
            reference = [
                t.clone()
                for t in (
                    expected.decode_swa_indices,
                    expected.decode_swa_lens,
                    expected.is_valid_token,
                )
            ]
